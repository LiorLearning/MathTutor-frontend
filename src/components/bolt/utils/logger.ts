...
let currentLevel: DebugLevel = process.env.NEXT_PUBLIC_VITE_LOG_LEVEL ?? process.env.NEXT_PUBLIC_DEV ? 'debug' : 'info';
...
function setLevel(level: DebugLevel) {
  if ((level === 'trace' || level === 'debug') && process.env.NEXT_PUBLIC_PROD) {
    return;
  }
